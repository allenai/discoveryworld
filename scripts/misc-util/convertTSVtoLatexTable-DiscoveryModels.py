...
atAverage = False
curHighlightColor = "FFFFFF"
for lineNum, line in enumerate(lines):
    print("line: " + str(line))
    if (lineNum % 4 == 0) and (len(line[0]) > 0):
        # Swap highlight color
        if (curHighlightColor == "F3F3F3"):
            curHighlightColor = "FFFFFF"
        else:
            curHighlightColor = "F3F3F3"

        print("line: " + str(line))
        topic = line[header.index('#')]
        taskDesc = line[header.index('Task')]
        latex_table += f"\\rowcolor[HTML]{{{curHighlightColor}}} \n"
...
filenameOut = "table-discoverymodels.tex"
