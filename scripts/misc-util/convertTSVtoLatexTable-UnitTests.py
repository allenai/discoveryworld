...
file_path = "paper-results/DiscoveryWorld Scenarios - Unit Test Results.tsv"
...
# Header
# Remove the first 3 lines (they're useful for humans to read the google sheet, but the header starts on line 4)
data = data[3:]
...
latex_table = r"""
%
%   Table: Unit Test Results (Unit Tests)
%
\begin{table*}[!t]
...
for lineNum, line in enumerate(lines):
    ...
    #latex_table += """\multicolumn{3}{l}{""" + difficulty + """}}"""
    color_row = 'F3F3F3' if lineNum % 2 == 0 else 'FFFFFF'
    latex_table += r"\rowcolor[HTML]{" + color_row + "}\n"
    row_num = ""
    try:
        row_num = int(topic)
    except:
        pass
    latex_table += f"{row_num}  & {difficulty} "
...
filenameOut = "table-unittests.tex"
with open(filenameOut, 'w') as file:
    file.write(latex_table)

print("LaTeX table generated and saved as " + str(filenameOut))
