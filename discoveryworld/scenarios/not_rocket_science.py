...
def makeScenarioNotRocketScience(world, numUserAgents=1, difficulty="easy"):
    ...
    def _computeStatsForFuel(fuel, fuelType):
        ...
        #scoringInfo["criticalHypotheses"].append(f"The wet mass needed for the rocket with fuel {fuelType} would be {fuelMassNeeded} kg ({fuelMassNeeded/rocketWithFuelMass:%} of total mass).")
    ...
    def updateTick(self):
        ...
        if not self.scorecardUseLaunchTerminal.completed:
            if "interactedWith" in launchTerminal.attributes["states"]:
                self.scorecardUseLaunchTerminal.updateScore(1, True, associatedUUIDs=[agent.uuid, launchTerminal.uuid], associatedNotes=f"Agent (UUID: {agent.uuid}) has used the launch terminal (UUID: {launchTerminal.uuid}).")
        ...
        if self.scoringInfo["difficulty"] == "challenge":
            ...
            if not self.scorecardUseLoadCellInterface.completed:
                if "interactedWith" in loadCell.attributes["states"]:
                    self.scorecardUseLoadCellInterface.updateScore(1, True, associatedUUIDs=[agent.uuid, loadCell.uuid], associatedNotes=f"Agent (UUID: {agent.uuid}) has used the load cell interface (UUID: {loadCell.uuid}).")
